+++
title = "Basic Income"
slug = "basic-income"
description = "Basic income as the automation dividend: stability, freedom, simplification."
template = "grundeinkommen.html"

[extra]
show_cta = true
cta_text = "Convinced? Get involved and help build the case for basic income."
faq_title = "Frequently asked questions"
faq = [
  { q = "Isn't this just 'money for everyone'?", a = "No. Basic income replaces a large part of today's social insurance bureaucracy with a simple, universal basic security. It is a simplification of the system — not a handout." },
  { q = "How is it financed?", a = "Not through a special robot tax — automation is not punished. It is financed through the normal taxation of growing value creation (income, consumption, wealth), through simplification gains in the social system, and by reallocating existing transfers — exactly the levers our UBI Calculator models. Concrete models are developed with economists and social scientists." },
  { q = "Will people stop working?", a = "Research shows: no. Pilot projects worldwide demonstrate that people with basic security start businesses more often, pursue further education and work more productively — not less." },
  { q = "Why now?", a = "Automation is accelerating. Without a stabilising foundation, social disruption threatens. Basic income is a preventive measure, not a reactive one." },
  { q = "Does it apply to non-Swiss residents?", a = "The exact design — such as residency requirements and contribution periods — will be determined through democratic process. The key is the principle of universal basic security." },
  { q = "What happens to existing social benefits?", a = "Basic income replaces and simplifies existing transfer payments. The goal is not to add more on top, but to make the system simpler, more transparent and fairer." },
  { q = "Why a pilot and not immediate implementation?", a = "Because we are pragmatic. A federal pilot compares models (UBI, negative income tax, hybrid), measures impact and creates a solid basis for decision-making — instead of ideology." },
  { q = "What is measured?", a = "Three core areas: well-being and security (existential stress, stability), entrepreneurship and startups, and bureaucracy costs and administrative burden." },
]

# Calculator
sim_title = "UBI Calculator"
sim_desc = "What does an unconditional basic income cost for Switzerland — and how can it be funded? Adjust the parameters and compare scenarios."
scenario_referendum = "2016 Referendum"
scenario_modest = "Modest Start"
scenario_full = "Full Transition"
panel_parameters = "Parameters"
panel_results = "Results"
slider_ubi_adult = "Basic income adults (CHF/mo.)"
slider_ubi_child = "Basic income children (CHF/mo.)"
slider_income_tax = "Income tax surcharge (%)"
slider_vat = "VAT increase (pp.)"
slider_wealth_tax = "Wealth tax (%)"
slider_savings = "Social system savings (%)"
result_annual_cost = "Annual cost"
result_revenue = "Revenue"
result_income_tax_revenue = "Income tax"
result_vat_revenue = "VAT"
result_wealth_tax_revenue = "Wealth tax"
result_savings_revenue = "Savings"
result_net = "Balance"
result_surplus = "Surplus"
result_deficit = "Deficit"
result_per_month = "per month"
result_per_year = "per year"
result_billion = "bn."
impact_title = "Net effect per person"
impact_subtitle = "Monthly income"
impact_net_gain = "Net gain"
impact_net_loss = "Net loss"
impact_ubi_received = "UBI received"
impact_additional_taxes = "Additional taxes"
result_total_revenue = "Total revenue"
compare_title = "Comparison"
compare_current = "Current social spending"
compare_ubi_cost = "UBI cost"
methodology_title = "Methodology & assumptions"
methodology_text = "This model is a simplification. It is based on publicly available data from the Swiss Federal Statistical Office (FSO) and the Federal Tax Administration (FTA). Actual funding depends on behavioural effects, economic dynamics, and political decisions that a static model cannot capture."
sources_title = "Data sources"
sources_intro = "All values used in the calculator are based on publicly available Swiss statistics. Values are rounded."
sources_col_param = "Parameter"
sources_col_value = "Value"
sources_col_source = "Source"

[[extra.sections]]
title = "The Automation Dividend"
text = "When machines produce a growing share of value, part of this dividend must flow back to society. Basic income is not charity — it is a structural answer to a structural change."

[[extra.sections]]
title = "What It Enables"
text = ""
items = [
  { label = "Freedom to Retrain", text = "Those who lose their job need a bridge, not fear." },
  { label = "Entrepreneurship-friendly", text = "A stable foundation enables risk-taking for new ideas." },
  { label = "Recognising Care Work", text = "Unpaid care work is made visible through financial security." },
  { label = "Cutting Bureaucracy", text = "Dozens of individual benefits are replaced by a transparent system." },
  { label = "Flexible Work", text = "Part-time, project work or further education become possible without existential fear." },
]

[[extra.sections]]
title = "Not an Experiment — an Evolution"
text = "Switzerland has one of the best social systems in the world. Basic income builds on it: simpler, more direct, more contemporary. It is the logical evolution for a society in transition."

[[extra.sources]]
param = "Resident population (18+)"
value = "7.53m"
source = "FSO, STATPOP 2024"
url = "https://www.bfs.admin.ch/bfs/en/home/statistics/population/effectif-evolution.html"

[[extra.sources]]
param = "Resident population (<18)"
value = "1.53m"
source = "FSO, STATPOP 2024"
url = "https://www.bfs.admin.ch/bfs/en/home/statistics/population/effectif-evolution.html"

[[extra.sources]]
param = "Taxable income"
value = "CHF 420bn"
source = "FTA, Tax statistics natural persons 2021"
url = "https://www.estv.admin.ch/estv/en/home/the-fta/tax-statistics-fta/key-figures.html"

[[extra.sources]]
param = "Household consumption expenditure"
value = "CHF 380bn"
source = "FSO, National accounts 2023"
url = "https://www.bfs.admin.ch/bfs/en/home/statistics/national-economy/national-accounts.html"

[[extra.sources]]
param = "Net taxable wealth"
value = "CHF 2,800bn"
source = "FTA, Wealth statistics natural persons 2021"
url = "https://www.estv.admin.ch/estv/en/home/the-fta/tax-statistics-fta/key-figures.html"

[[extra.sources]]
param = "Social expenditure"
value = "CHF 185bn"
source = "FSIO, General account of social insurance 2022"
url = "https://www.bsv.admin.ch/bsv/en/home/social-insurance/ueberblick/grsv.html"
+++
